...
- 需要 GitHub 同步或 Sites 部署时：仓库可见性 `Public` 或 `Private`；用户未说明时询问一次，已经说明时不重复询问。
...
部署前列出全部下载附件。公开站点的正文、图表和 `public/downloads/` 文件均视为公开，与 GitHub 仓库是 Public 还是 Private 无关。存在敏感或权限不明文件时必须暂停公开部署并请求确认。
